...
def testData = findTestData('IMONE/IM_Esports/Thunder_Fire')
...
for (int i = 1; i <= rowCount; i++) {
    ...
    response = WS.sendRequest(findTestObject('Execution/Execution Loop/ESPORTS/Thunderfire/Game_NewLaunchMobileGame', [('baseUrl') : GlobalVariable.baseUrl
                , ('MerchantCode') : GlobalVariable.MerchantCode, ('PlayerId') : GlobalVariable.PlayerId, ('GameCode') : gameCode
                , ('LANGUAGE') : findTestData('IMONE/IM_Esports/Thunder_Fire').getValue(8, 1), ('ProductWallet') : findTestData(
                    'IMONE/IM_Esports/Thunder_Fire').getValue(3, 1), ('Currency') : findTestData('IMONE/IM_Esports/Thunder_Fire').getValue(
                    6, 1)]))
