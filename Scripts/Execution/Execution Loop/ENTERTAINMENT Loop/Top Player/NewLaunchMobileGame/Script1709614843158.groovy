...
def testData = findTestData('IMONE/IM_Entertainment/Top_Player')
...
for (int i = 1; i <= rowCount; i++) {
    ...
    response = WS.sendRequest(findTestObject('Execution/Execution Loop/ENTERTAINMENT Loop/Top Player/Game_NewLaunchMobileGame', 
            [('baseUrl') : GlobalVariable.baseUrl, ('MerchantCode') : GlobalVariable.MerchantCode, ('PlayerId') : GlobalVariable.PlayerId
                , ('GameCode') : gameCode, ('LANGUAGE') : findTestData('IMONE/IM_Entertainment/Top_Player').getValue(8, 
                    1), ('ProductWallet') : findTestData('IMONE/IM_Entertainment/Top_Player').getValue(3, 1), ('Currency') : findTestData(
                    'IMONE/IM_Entertainment/Top_Player').getValue(6, 1)]))
