...
def testData = findTestData('IMONE/IM_LiveDealer/Pragmatic_Play_LD')
...
for (int i = 1; i <= rowCount; i++) {
    ...
    response = WS.sendRequest(findTestObject('Execution/Execution Loop/LIVE DEALER Loop/Pragmatic Play LD/Game_NewLaunchMobileGame', 
            [('baseUrl') : GlobalVariable.baseUrl, ('MerchantCode') : GlobalVariable.MerchantCode, ('PlayerId') : GlobalVariable.PlayerId
                , ('GameCode') : gameCode, ('LANGUAGE') : findTestData('IMONE/IM_LiveDealer/Pragmatic_Play_LD').getValue(
                    8, 1), ('ProductWallet') : findTestData('IMONE/IM_LiveDealer/Pragmatic_Play_LD').getValue(3, 1), ('Currency') : findTestData(
                    'IMONE/IM_LiveDealer/Pragmatic_Play_LD').getValue(6, 1)]))
