...
def testData = findTestData('IMONE/IM_LiveDealer/World_Ent_EBET')
...
for (int i = 1; i <= rowCount; i++) {
    ...
    response = WS.sendRequest(findTestObject('Execution/Execution Loop/LIVE DEALER Loop/World Entertainment (EBET)/Game_NewLaunchMobileGame', 
            [('baseUrl') : GlobalVariable.baseUrl, ('MerchantCode') : GlobalVariable.MerchantCode, ('PlayerId') : GlobalVariable.PlayerId
                , ('GameCode') : gameCode, ('LANGUAGE') : findTestData('IMONE/IM_LiveDealer/World_Ent_EBET').getValue(8, 
                    1), ('ProductWallet') : findTestData('IMONE/IM_LiveDealer/World_Ent_EBET').getValue(3, 1), ('Currency') : findTestData(
                    'IMONE/IM_LiveDealer/World_Ent_EBET').getValue(6, 1)]))
