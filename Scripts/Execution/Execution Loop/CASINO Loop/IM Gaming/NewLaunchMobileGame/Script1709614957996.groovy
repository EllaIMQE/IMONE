...
def testData = findTestData('IMONE/IM_Casino/IM_Gaming')
...
for (int i = 1; i <= rowCount; i++) {
    ...
    response = WS.sendRequest(findTestObject('Execution/Execution Loop/CASINO Loop/IM Gaming/Game_NewLaunchMobileGame', 
            [('baseUrl') : GlobalVariable.baseUrl, ('MerchantCode') : GlobalVariable.MerchantCode, ('PlayerId') : GlobalVariable.PlayerId
                , ('GameCode') : gameCode, ('LANGUAGE') : findTestData('IMONE/IM_Casino/IM_Gaming').getValue(8, 1), ('ProductWallet') : findTestData(
                    'IMONE/IM_Casino/IM_Gaming').getValue(3, 1), ('Currency') : findTestData('IMONE/IM_Casino/IM_Gaming').getValue(
                    6, 1)]))
