...
def testData = findTestData('IMONE/IM_Financial/Fin_Game')
...
for (int i = 1; i <= rowCount; i++) {
    ...
    response = WS.sendRequest(findTestObject('Execution/Execution Loop/FINANCIAL Loop/Fin Game/Game_NewLaunchMobileGame', 
            [('baseUrl') : GlobalVariable.baseUrl, ('MerchantCode') : GlobalVariable.MerchantCode, ('PlayerId') : GlobalVariable.PlayerId
                , ('GameCode') : gameCode, ('LANGUAGE') : findTestData('IMONE/IM_Financial/Fin_Game').getValue(8, 1), ('ProductWallet') : findTestData(
                    'IMONE/IM_Financial/Fin_Game').getValue(3, 1), ('Currency') : findTestData('IMONE/IM_Financial/Fin_Game').getValue(
                    6, 1)]))
