...
def testData = findTestData('IMONE/IM_BoardGames/Meitian')
...
for (int i = 1; i <= rowCount; i++) {
    ...
    response = WS.sendRequest(findTestObject('Execution/Execution Loop/BOARDGAMES Loop/Meitian Gaming/Game_NewLaunchMobileGame', 
            [('baseUrl') : GlobalVariable.baseUrl, ('MerchantCode') : GlobalVariable.MerchantCode, ('PlayerId') : GlobalVariable.PlayerId
                , ('GameCode') : gameCode, ('LANGUAGE') : findTestData('IMONE/IM_BoardGames/Meitian').getValue(8, 2), ('ProductWallet') : findTestData(
                    'IMONE/IM_BoardGames/Meitian').getValue(3, 1), ('Currency') : findTestData('IMONE/IM_BoardGames/Meitian').getValue(
                    6, 1)]))
