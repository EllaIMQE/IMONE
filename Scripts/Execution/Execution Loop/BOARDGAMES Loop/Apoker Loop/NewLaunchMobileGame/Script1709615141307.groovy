...
def testData = findTestData('IMONE/IM_Slot/Advant_Play')
...
for (int i = 1; i <= rowCount; i++) {
    ...
    response = WS.sendRequest(findTestObject('Execution/Execution Loop/BOARDGAMES Loop/APOKER/Game_NewLaunchMobileGame', 
            [('baseUrl') : GlobalVariable.baseUrl, ('MerchantCode') : GlobalVariable.MerchantCode, ('PlayerId') : GlobalVariable.PlayerId
                , ('GameCode') : gameCode, ('LANGUAGE') : findTestData('IMONE/IM_BoardGames/Apoker').getValue(8, 1), ('ProductWallet') : findTestData(
                    'IMONE/IM_BoardGames/Apoker').getValue(3, 1), ('Currency') : findTestData('IMONE/IM_BoardGames/Apoker').getValue(
                    6, 1)]))
