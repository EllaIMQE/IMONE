...
def testData = findTestData('IMONE/IM_BoardGames/Yan_You')
...
for (int i = 1; i <= rowCount; i++) {
    ...
    def gameName = testData.getValue('Chinese Name', i)

    println("Launching game with code: $gameCode")

    response = WS.sendRequest(findTestObject('Execution/Execution Loop/BOARDGAMES Loop/Yan You Gaming/Game_NewLaunchGame', 
            [('baseUrl') : GlobalVariable.baseUrl, ('MerchantCode') : GlobalVariable.MerchantCode, ('PlayerId') : GlobalVariable.PlayerId
                , ('GameCode') : gameCode, ('LANGUAGE') : findTestData('IMONE/IM_BoardGames/Yan_You').getValue(8, 2), ('ProductWallet') : findTestData(
                    'IMONE/IM_BoardGames/Yan_You').getValue(3, 1), ('Currency') : findTestData('IMONE/IM_BoardGames/Yan_You').getValue(
                    6, 1)]))
