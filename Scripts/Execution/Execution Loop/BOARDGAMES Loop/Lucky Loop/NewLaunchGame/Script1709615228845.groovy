...
def testData = findTestData('IMONE/IM_BoardGames/Lucky')
...
for (int i = 1; i <= rowCount; i++) {
    ...
    response = WS.sendRequest(findTestObject('Execution/Execution Loop/BOARDGAMES Loop/Lucky Gaming/Game_NewLaunchGame', 
            [('baseUrl') : GlobalVariable.baseUrl, ('MerchantCode') : GlobalVariable.MerchantCode, ('PlayerId') : GlobalVariable.PlayerId
                , ('GameCode') : gameCode, ('LANGUAGE') : findTestData('IMONE/IM_BoardGames/Lucky').getValue(8, 2), ('ProductWallet') : findTestData(
                    'IMONE/IM_BoardGames/Lucky').getValue(3, 1), ('Currency') : findTestData('IMONE/IM_BoardGames/Lucky').getValue(
                    6, 1)]))
