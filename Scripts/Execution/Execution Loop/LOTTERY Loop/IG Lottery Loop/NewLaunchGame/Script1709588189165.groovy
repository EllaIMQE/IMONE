...
for (int i = 1; i <= rowCount; i++) {
    ...
    response = WS.sendRequest(findTestObject('Execution/Execution Loop/LOTTERY Loop/IG Lottery/Game_NewLaunchGame', [('baseUrl') : GlobalVariable.baseUrl, 
		('MerchantCode') : GlobalVariable.MerchantCode, 
				('PlayerId') : GlobalVariable.PlayerId, 
				('GameCode') : gameCode, 
				('LANGUAGE') : findTestData('IMONE/IM_Lottery/IG_Lottery').getValue(8, 1), 
				('ProductWallet') : findTestData('IMONE/IM_Lottery/IG_Lottery').getValue(3, 1), 
				('Currency') : findTestData('IMONE/IM_Lottery/IG_Lottery').getValue(6, 1)]))
