...
def testData = findTestData('IMONE/IM_Lottery/SG_Win')
...
for (int i = 1; i <= rowCount; i++) {
    ...
    response = WS.sendRequest(findTestObject('Execution/Execution Loop/LOTTERY Loop/SG Win Lottery/Game_NewLaunchMobileGame', 
            [('baseUrl') : GlobalVariable.baseUrl, ('MerchantCode') : GlobalVariable.MerchantCode, ('PlayerId') : GlobalVariable.PlayerId
                , ('GameCode') : gameCode, ('LANGUAGE') : findTestData('IMONE/IM_Lottery/SG_Win').getValue(8, 1), ('ProductWallet') : findTestData(
                    'IMONE/IM_Lottery/SG_Win').getValue(3, 1), ('Currency') : findTestData('IMONE/IM_Lottery/SG_Win').getValue(
                    6, 1)]))
