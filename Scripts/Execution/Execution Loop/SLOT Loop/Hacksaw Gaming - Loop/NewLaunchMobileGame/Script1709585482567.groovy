...
def testData = findTestData('IMONE/IM_Slot/Hacksaw')
...
for (int i = 1; i <= rowCount; i++) {
    ...
    response = WS.sendRequest(findTestObject('Execution/Execution Loop/SLOT Loop/Hacksaw Gaming/Game_NewLaunchMobileGame', 
            [('baseUrl') : GlobalVariable.baseUrl, ('MerchantCode') : GlobalVariable.MerchantCode, ('PlayerId') : GlobalVariable.PlayerId
                , ('GameCode') : gameCode, ('LANGUAGE') : findTestData('IMONE/IM_Slot/Hacksaw').getValue(8, 1), ('ProductWallet') : findTestData(
                    'IMONE/IM_Slot/Hacksaw').getValue(3, 1), ('Currency') : findTestData('IMONE/IM_Slot/Hacksaw').getValue(
                    6, 1)]))
    ...
    WebUI.enhancedClick(findTestObject('Execution/IMONE SLOT/Hacksaw Gaming/Cubes 2/canvas'), FailureHandling.OPTIONAL)
