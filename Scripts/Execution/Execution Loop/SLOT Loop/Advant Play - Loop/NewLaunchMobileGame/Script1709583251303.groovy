...
for (int i = 1; i <= rowCount; i++) {
    ...
    response = WS.sendRequest(findTestObject('Execution/Execution Loop/SLOT Loop/Advant Play/Game_NewLaunchMobileGame', 
            [('baseUrl') : GlobalVariable.baseUrl, ('MerchantCode') : GlobalVariable.MerchantCode, ('PlayerId') : GlobalVariable.PlayerId
                , ('GameCode') : gameCode, ('LANGUAGE') : findTestData('IMONE/IM_Slot/Advant_Play').getValue(8, 1), ('ProductWallet') : findTestData(
                    'IMONE/IM_Slot/Advant_Play').getValue(3, 1), ('Currency') : findTestData('IMONE/IM_Slot/Advant_Play').getValue(
                    6, 1)]))
