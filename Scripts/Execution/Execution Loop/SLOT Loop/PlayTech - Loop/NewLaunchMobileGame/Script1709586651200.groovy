...
def testData = findTestData('IMONE/IM_Slot/Playtech')
...
for (int i = 1; i <= rowCount; i++) {
    ...
    response = WS.sendRequest(findTestObject('Execution/Execution Loop/SLOT Loop/PlayTech/Game_NewLaunchMobileGame', [('baseUrl') : GlobalVariable.baseUrl
                , ('MerchantCode') : GlobalVariable.MerchantCode_Playtech, ('PlayerId') : GlobalVariable.PlayerId, ('GameCode') : gameCode
                , ('LANGUAGE') : findTestData('IMONE/IM_Slot/Playtech').getValue(8, 1), ('ProductWallet') : findTestData(
                    'IMONE/IM_Slot/Playtech').getValue(3, 1), ('Currency') : findTestData('IMONE/IM_Slot/Playtech').getValue(
                    6, 1)]))
