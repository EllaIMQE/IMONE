...
WebUI.callTestCase(findTestCase('Config/Generate Random Number'), [:], FailureHandling.STOP_ON_FAILURE)
...
def requestBody = new JsonBuilder([('MerchantCode') : MerchantCode, ('PlayerId') : PlayerId, ('ProductWallet') : ProductWallet
    , ('TransactionId') : transactionId, ('Amount') : Amount]).toPrettyString()

def builder = new RestRequestObjectBuilder()

def requestObject = builder.withRestRequestMethod('POST').withRestUrl('http://operatorapi-staging.imaegisapi.com/Transaction/PerformTransfer').withHttpHeaders(
    [new TestObjectProperty('Content-Type', ConditionType.EQUALS, 'application/json')]).withRestParameters([new TestObjectProperty(
        'MerchantCode', ConditionType.EQUALS, MerchantCode), new TestObjectProperty('PlayerId', ConditionType.EQUALS, 
        PlayerId), new TestObjectProperty('ProductWallet', ConditionType.EQUALS, ProductWallet), new TestObjectProperty(
        'TransactionId', ConditionType.EQUALS, transactionId), new TestObjectProperty('Amount', ConditionType.EQUALS, Amount)]).withTextBodyContent(
    requestBody).build()
